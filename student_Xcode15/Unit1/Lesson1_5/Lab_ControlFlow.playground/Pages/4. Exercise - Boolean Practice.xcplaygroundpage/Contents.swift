...
let hasPizza = false
let hasVegan = true
if resturant = 2 attributes {
 print(Let's go!)
      } else {
  print(Sorry, we'll have to think of somewhere elseelse!)
       }
...
let isRaining = true
let isSunny = true
  let isNiceWeather = temp || isRaining || isSunny
  if isNiceWeather = temp || isRaining || isSunny {
